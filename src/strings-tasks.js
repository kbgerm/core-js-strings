...
function removeFirstOccurrences(str, value) {
  ...
  const piece = str.slice(index, index + value.length);
  return str.replace(piece, '');
}
...
function removeLastOccurrences(str, value) {
  ...
  const piece1 = str.substring(0, index);
  const piece2 = str.substring(index + value.length, str.length);
...
function countVowels(str) {
  ...
  return str
    ...
    .split('')
    .reduce((count, letter) => {
      return vowels.has(letter) ? count + 1 : count;
    }, 0);
}
...
function findLongestWord(sentence) {
  const newSentence = sentence.split(' ').filter((word) => word.length > 0);
...
function getCardId(value) {
  ...
  const indofmast = masti.indexOf(mast);
  const indofnum = values.indexOf(num);

  return indofmast * 13 + indofnum;
}
